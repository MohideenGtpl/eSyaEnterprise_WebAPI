   ...
   public class ReportHeaderRepository:IReportHeaderRepository
    {
        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveGeneralReport(bool status, int Businesskey, int ReportHeaderId)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEcgrrh rpt = db.GtEcgrrh.Where(w => w.BusinessKey == Businesskey && w.ReportHeader==ReportHeaderId).FirstOrDefault();
                        if (rpt == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Report Header is not exist" };
                        }

                        rpt.ActiveStatus = status;
                        await db.SaveChangesAsync();
                        dbContext.Commit();

                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Report Header Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Report Header De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
