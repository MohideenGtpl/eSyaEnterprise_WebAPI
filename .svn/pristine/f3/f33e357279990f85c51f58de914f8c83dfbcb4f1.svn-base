...
namespace eSyaConfigSetup.DL.Repository
{
    public class DepartmentCodesRepository:IDepartmentCodesRepository
    {
        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveDepartmentCodes(bool status, int deptId)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEadpcd dept = db.GtEadpcd.Where(w => w.DepartmentId == deptId).FirstOrDefault();
                        if (dept == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Department code is not exist" };
                        }
                        ...
                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Department code Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Department code De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
        ...
        #endregion
        #region Departement Location Link
        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveDepartmentLocationLink(bool status, int deptId, int deptlocId, int Businesskey)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEadpll deptloc = db.GtEadpll.Where(w => w.DepartmentId == deptId && w.DeptLocnId==deptlocId && w.BusinessKey==Businesskey).FirstOrDefault();
                        if (deptloc == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Department Location is not exist" };
                        }

                        deptloc.ActiveStatus = status;
                        await db.SaveChangesAsync();
                        dbContext.Commit();

                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Department Location Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Department Location De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
        ...
        #endregion Departement Location Link
    }
}
