...
namespace eSyaConfigSetup.DL.Repository
{
    public class TaxRuleRepository:ITaxRuleRepository
    {
        ...
        public async Task<DO_ReturnParameter> InsertIntoTaxRule(DO_TaxRule obj)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        var _isTaxShortCodeExist = db.GtEccntr.Where(w => w.Isdcode == obj.ISDCode && w.TaxCode == obj.TaxCode && w.TaxShortCode == obj.TaxShortCode && w.SplitCategoryPerc == obj.SplitCategoryPerc).Count();
                        ...
                        var _isTaxDescriptionExist = db.GtEccntr.Where(w => w.Isdcode == obj.ISDCode && w.TaxCode == obj.TaxCode && w.TaxDescription == obj.TaxDescription && w.SplitCategoryPerc == obj.SplitCategoryPerc).Count();
                        ...
                        int _maxSerialNumber = db.GtEccntr.Select(c => c.SerialNumber).DefaultIfEmpty().Max();
                        _maxSerialNumber = _maxSerialNumber + 1;
                        

                        var ap_cd = new GtEccntr
                        {
                            Isdcode = obj.ISDCode,
                            TaxCode = obj.TaxCode,
        ...
        public async Task<DO_ReturnParameter> UpdateTaxRule(DO_TaxRule obj)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEccntr ap_cd = db.GtEccntr.Where(w => w.Isdcode == obj.ISDCode && w.TaxCode == obj.TaxCode && w.SerialNumber == obj.SerialNumber).FirstOrDefault();
                        if (ap_cd == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Tax Rule is not exist For Selected Tax Code." };
                        }

                        IEnumerable<GtEccntr> ls_apct = db.GtEccntr.Where(w => w.Isdcode == obj.ISDCode && w.TaxCode == obj.TaxCode).ToList();
        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveTaxRule(bool status, int Isd_code, int Taxcode,int serialNumber)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEccntr tax_rule = db.GtEccntr.Where(w => w.Isdcode == Isd_code && w.TaxCode == Taxcode && w.SerialNumber == serialNumber).FirstOrDefault();
                        if (tax_rule == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Tax Rule is not exist" };
                        }

                        tax_rule.ActiveStatus = status;
                        await db.SaveChangesAsync();
                        dbContext.Commit();

                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Tax Rule Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Tax Rule De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
