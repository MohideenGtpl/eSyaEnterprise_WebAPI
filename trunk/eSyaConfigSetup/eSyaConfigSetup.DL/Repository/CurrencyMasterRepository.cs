        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveCurrencyMaster(bool status, string currency_code)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEccuco cuurency_code = db.GtEccuco.Where(w => w.CurrencyCode.ToUpper().Replace(" ", "") == currency_code.ToUpper().Replace(" ", "")).FirstOrDefault();
                        ...
                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Currency code Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Currency code De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
        ...
        public async Task<DO_ReturnParameter> ActiveOrDeActiveCurrencyDenomination(bool status, string currencycode, int denomId)
        {
            using (var db = new eSyaEnterprise())
            {
                using (var dbContext = db.Database.BeginTransaction())
                {
                    try
                    {
                        GtEccudn cu_denomination = db.GtEccudn.Where(w => w.CurrencyCode.ToUpper().Replace(" ", "") == currencycode.ToUpper().Replace(" ", "") && w.DenomId==denomId).FirstOrDefault();
                        if (cu_denomination == null)
                        {
                            return new DO_ReturnParameter() { Status = false, Message = "Currency Denomination is not exist" };
                        }
                        ...
                        if (status == true)
                            return new DO_ReturnParameter() { Status = true, Message = "Currency Denomination Activated Successfully." };
                        else
                            return new DO_ReturnParameter() { Status = true, Message = "Currency Denomination De Activated Successfully." };
                    }
                    catch (DbUpdateException ex)
                    {
                        dbContext.Rollback();
        ...
        public List<DO_ExchangeRate> GetCurrencyExchangeRate(string Currencycode, int Businesskey)
        {
            try
            {
                ...
                using (var db = new eSyaEnterprise())
                {
                    ...
                    var cur_codes = db.GtEcbssc.Where(c => c.TocurrencyCode.ToUpper().Trim() != Currencycode.ToUpper().Trim()).Join(db.GtEccuco,
                          x => new { CurrencyCode = x.TocurrencyCode } ,
